...
class Queue extends Command {
     ...
     * @var string
     */
    const QUEUE_LISTEN = 'myunet:queue listen';
    ...
    {
        if (count($result = $this->process->thinkQuery('myunet:queue')) < 1) {
    ...
    {
        $items = $this->process->thinkQuery('myunet:queue');
    ...
    protected function cleanAction()
    {
        ...
        $clean = SystemQueue::where('exec_time', '<', time() - $days * 24 * 3600)->delete();
        ...
        [$timeout, $loops, $total] = [0, 0, SystemQueue::whereOr([$map1, $map2])->count()];
        foreach (SystemQueue::whereOr([$map1, $map2])->cursor() as $queue) {
    ...
    private function createListenProcess()
    {
        ...
        while (true) {
            ...
            foreach (SystemQueue::where($map)->order('exec_time asc')->cursor() as $queue) try {
                $args = "myunet:queue dorun {$queue['code']} -";
                $str = ">$ {$this->process->think($args)}";
                $this->output->comment($str);
    ...
    protected function doRunAction()
    {
        ...
        if (empty($this->code)) {
            ...
            if (empty($this->queue->record) || intval($this->queue->record->getAttr('status')) !== static::STATE_WAIT) {
                ...
                // 锁定任务状态，防止任务再次被执行
                SystemQueue::strict(false)->where(['code' => $this->code])->inc('attempts')->update([
    ...
    private function updateQueue(int $status, string $message, bool $isSplit = true)
    {
        ...
        SystemQueue::strict(false)->where(['code' => $this->code])->update([
